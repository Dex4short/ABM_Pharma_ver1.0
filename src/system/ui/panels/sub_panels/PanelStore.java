...
public class PanelStore extends UI3 implements Store{
	...
	public PanelStore() {
		...
		table_product_aisle = new TableAisle() {
			...
			public void onAddProductToCart(Packaging[] extracted_packs, Packaging sub_pack) { 
				addToCart(getSelectedProduct(), extracted_packs, sub_pack); 
			}
		};
		...
		table_product_cart = new TableCart() {
			...
			@Override
			public void onRemoveOrderFromCart(Packaging[] extracted_packs, Packaging sub_pack) {
				removeFromCart(getSelectedOrder(),extracted_packs, sub_pack);
			}
		};
	...
	public void onOpenCart(Cart cart) {
		table_product_cart.setCart(cart);
	}
	@Override
