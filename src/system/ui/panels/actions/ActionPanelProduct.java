...
public abstract class ActionPanelProduct  extends ActionPanel{
	...
	public ActionPanelProduct() {
		...
		setProducts(new Product[] {new Product()});
	...
	public void setProducts(Product products[]) {
		...
		while(r<rows.length) {
			rows[r] = new ProductRow(new Product());
			for(int c=0; c<7; c++) {
				rows[r].setCell(new Cell(), c);
			}
	...
	public void setProduct(Product product, int row) {
		
	}
	
	public abstract void onProductOk(Product products[]);
