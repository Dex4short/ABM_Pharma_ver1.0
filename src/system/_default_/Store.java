	...
	public default void addToCart(Cart cart, Product main_product, Packaging[] extracted_packs, Packaging sub_pack) {		
		...
		//if(main_product.getPackaging().getParentPackId() == -1) {
			...
			for(int n=0; n<product_children.length; n++) {
				extracted_packs[n+1].setPackId(product_children[n].getPackaging().getPackId());
				
				MySQL_Packaging.updatePackaging(extracted_packs[n+1]);
		...
		onAddToCart(MySQL_Orders.insertOrder(new Order(cart.getOrderNo(), sub_product)));
	}
