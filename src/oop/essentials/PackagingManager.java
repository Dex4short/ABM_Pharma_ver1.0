...
public class PackagingManager {
	...
	public static Packaging[] extract(Packaging main_pack, Packaging sub_pack) {
		...
		BigDecimal 
		length = new BigDecimal(mainPack_uom.getUnitSize()),
		scale = new BigDecimal(mainPack_uom.getUnitSize()),
		magnitude = null;
	...
	private static class PackagingData {
		...
		public Packaging createPackaging() {
			return new Packaging(-1, quantity, uom, -1, PackagingLine.Descendant, -1);
		}
		public String toString() {
			return  
				quantity_a.toString() + " " + uom.getUnitType().name() + 
				" - " +
				quantity_b.toString() + " " + uom.getUnitType().name() +
				" = " +  
				quantity.toStringQuantityAndSize() + " " + uom.getUnitType().name() ;
		}
	}
}
